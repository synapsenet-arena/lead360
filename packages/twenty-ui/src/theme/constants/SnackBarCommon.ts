import { MAIN_COLORS } from '@ui/theme/constants/MainColors';
import { RGBA } from '@ui/theme/constants/Rgba';

export const SNACK_BAR_COMMON = {
  success: {
    color: MAIN_COLORS.turquoise,
    backgroundColor: RGBA(MAIN_COLORS.turquoise, 0.3),
  },
  error: {
    color: MAIN_COLORS.red,
    backgroundColor: RGBA(MAIN_COLORS.red, 0.3),
  },
  warning: {
    color: MAIN_COLORS.orange,
    backgroundColor: RGBA(MAIN_COLORS.orange, 0.3),
  },
  info: {
    color: MAIN_COLORS.blue,
    backgroundColor: RGBA(MAIN_COLORS.blue, 0.3),
  },
};
